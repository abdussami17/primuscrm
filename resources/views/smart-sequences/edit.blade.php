...
@section('title', 'Edit Smart Sequence')
...
@section('content')
<form method="POST" action="{{ route('smart-sequences.update', $smartSequence) }}" id="sequenceForm">
    @csrf
    @method('PUT')
    ...
    <div class="content">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                ...
                <h4 class="mb-0 mt-2" id="modalTitle">Edit: {{ $smartSequence->title }}</h4>
            </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" 
                               value="{{ old('title', $smartSequence->title) }}" required>
                        @error('title')
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="form-check form-switch mt-2">
                            ...
                            <input class="form-check-input" type="checkbox" name="is_active" value="1" 
                                   {{ old('is_active', $smartSequence->is_active) ? 'checked' : '' }}>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div id="criteriaContainer">
                    @foreach($smartSequence->criteriaGroups as $groupIndex => $group)
                        @include('smart-sequences.partials.criteria-group', [
                            'groupIndex' => $groupIndex,
                            'logicType' => $group->logic_type,
                            'criteria' => $group->criteria->map(function($c) {
                                return [
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div id="actionsContainer">
                    @foreach($smartSequence->actions as $actionIndex => $action)
                        @include('smart-sequences.partials.action-row', [
                            'actionIndex' => $actionIndex,
                            'action' => [
                                'action_type' => $action->action_type,
                                'delay_value' => $action->delay_value,
...
let groupCounter = {{ $smartSequence->criteriaGroups->count() }};
let actionCounter = {{ $smartSequence->actions->count() }};
...
function handleOperatorChange(selectElement) {
    ...
    if (!operator || operator === 'is_blank' || operator === 'is_not_blank') return;
    
    if (operator.includes('between')) {
        if (fieldType === 'date') {
            valueContainer.innerHTML = `<div class="d-flex gap-2 align-items-center">
...
function handleActionTypeChange(selectElement) {
    ...
    switch(actionType) {
        ...
        case 'change-status-lost':
            html = `<input type="hidden" name="${baseName}[status]" value="Lost">
        ...
        case 'change-lead-type':
            ...
            break;
        default:
            if (actionType.startsWith('change-') || actionType === 'reassign-lead') {
        ...
        <div class="row g-2">
            <div class="col-md-3">
                ...
                <select name="actions[__ACTION__][action_type]" class="form-select form-select-sm" required onchange="handleActionTypeChange(this)">
                    ...
                    @foreach(\App\Models\SequenceAction::getActionTypes() as $value => $label)
            ...
            <div class="col-md-2">
                ...
                <select name="actions[__ACTION__][delay_unit]" class="form-select form-select-sm">
                    @foreach(\App\Models\SequenceAction::getDelayUnits() as $value => $label)
                        <option value="{{ $value }}">{{ $label }}</option>
                    @endforeach
                </select>
            </div>
