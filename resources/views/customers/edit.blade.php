...
document.addEventListener('click', function (e) {
    ...
    if (callBtn) {
        ...
        }).finally(() => { callBtn.disabled = false; callBtn.innerHTML = '<i class="ti ti-phone"></i>'; });
    }
    if (smsBtn) {
        ...
        }).finally(() => { smsBtn.disabled = false; smsBtn.innerHTML = '<i class="ti ti-message-circle"></i>'; });
    }
    if (videoBtn) {
        ...
        fetch('/telnyx/video', {
            ...
            if (data && data.success) {
                ...
                try {
                    const join = data.data && (data.data.join_url || data.data.data && data.data.data.join_url);
        ...
        }).finally(() => { videoBtn.disabled = false; videoBtn.innerHTML = '<i class="ti ti-device-desktop"></i>'; });
    }
});
...
async function api(url, method = 'GET', data = null) {
    const options = {
        method,
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': AppState.csrfToken },
...
function showToast(a, b = 'success') {
    ...
    if (!message || lowerMsg === alertType || ['error','success','warning','info','danger'].includes(lowerMsg)) {
    ...
    if (typeof toastr !== 'undefined' && toastr[alertType]) {
        try { toastr[alertType](message); return; } catch(e) { /* fallback below */ }
    }
...
function selectDeal(dealId) {
    ...
    document.dispatchEvent(
        new CustomEvent('deal:selected', {
            detail: { dealId, customerId: AppState.customerId }
        })
    );
}
...
<div class="row g-3" data-customer-id="{{ $customerId }}">
  ...
  <div class="top-shortcut-btn ">
    ...
    <i class="ti ti-trash text-danger delete-customer" data-bs-toggle="tooltip" style="cursor:pointer;font-size: 18px;" aria-label="Delete Customer" data-bs-original-title="Delete Customer"></i>
    ...
    <div class="customer-social-icons d-flex align-items-center justify-content-end gap-2">
                <div class="customer-social-icon" data-platform="facebook" onclick="openCustomerSocialModal('facebook', {{ $customerId }})" title="Facebook">
            <i class="fab fa-facebook-f"></i>
        </div>
        <div class="customer-social-icon" data-platform="instagram" onclick="openCustomerSocialModal('instagram', {{ $customerId }})" title="Instagram">
      <i class="fab fa-instagram"></i>
    </div>
        <div class="customer-social-icon" data-platform="twitter" onclick="openCustomerSocialModal('twitter', {{ $customerId }})" title="Twitter/X">
      <i class="fab fa-x-twitter"></i>
    </div>
                <div class="customer-social-icon" data-platform="youtube" onclick="openCustomerSocialModal('youtube', {{ $customerId }})" title="YouTube">
            <i class="fab fa-youtube"></i>
        </div>
                <div class="customer-social-icon" data-platform="tiktok" onclick="openCustomerSocialModal('tiktok', {{ $customerId }})" title="TikTok">
            <i class="fab fa-tiktok"></i>
        </div>
                <div class="customer-social-icon" data-platform="reddit" onclick="openCustomerSocialModal('reddit', {{ $customerId }})" title="Reddit">
            <i class="fab fa-reddit-alien"></i>
        </div>
  </div>

                {{-- Profile Row --}}
            <form id="customerForm" method="POST" action="{{ route('customers.update',$customer->id) }}" enctype="multipart/form-data">
    @csrf
                    ...
                    <div class="row g-2 align-items-center">
                        ...
                        <div class="col-2 col-md-1 d-flex justify-content-center">
                            <i class="ti ti-ban do-not-contact-icon" 
                               ...
                               data-bs-toggle="modal" data-bs-target="#doNotContactModal"></i>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-3 row g-2">
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <div class="input-group">
                        ...
                        <button type="button" class="btn btn-outline-secondary telnyx-call-btn" data-phone="{{ $customer->cell_phone ?? '' }}">
                            <i class="ti ti-phone"></i>
                        </button>
                        <button type="button" class="btn btn-outline-secondary telnyx-sms-btn" data-phone="{{ $customer->cell_phone ?? '' }}">
                            <i class="ti ti-message-circle"></i>
                        </button>
                        <button type="button" class="btn btn-outline-secondary telnyx-video-btn" data-phone="{{ $customer->cell_phone ?? '' }}" title="Video Call">
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <div class="input-group">
                        ...
                        <button type="button" class="btn btn-outline-secondary telnyx-call-btn" data-phone="{{ $customer->work_phone ?? '' }}">
                            <i class="ti ti-phone"></i>
                        </button>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <div class="input-group">
                        ...
                        <button type="button" class="btn btn-outline-secondary telnyx-call-btn" data-phone="{{ $customer->home_phone ?? '' }}">
                            <i class="ti ti-phone"></i>
                        </button>
                    </div>
                </div>
                ...
                <div class="col-md-6 mb-2">
                    <div class="input-group">
                        ...
                        <a href="mailto:{{ $customer->email ?? '' }}" class="btn btn-outline-secondary"><i class="ti ti-mail"></i></a>
                    </div>
                </div>
                <div class="col-md-6 mb-2">
                    <div class="input-group">
                        ...
                        <a href="#" class="btn btn-outline-secondary"><i class="ti ti-mail"></i></a>
                    </div>
                </div>
            </div>
        </div>
...
<div class="col-12 mt-3">
    <div class="d-flex align-items-center gap-2">
        <button class="btn btn-sm btn-light  border border-1" type="button" id="toggleCoBuyerBtn" onclick="toggleEditCoBuyer()">
            {{ $customer?->coBuyer?->first_name ? 'Edit Co-Buyer' : '+ Co-Buyer' }}
        </button>

        <button type="button" id="confirmCoBuyerBtn" class="btn btn-sm btn-success {{ $customer?->coBuyer?->first_name ? '' : 'd-none' }}" title="Confirm Co-Buyer" onclick="confirmCoBuyer()">
            <i class="ti ti-check"></i>
        </button>

        <button type="button" id="deleteCoBuyerBtn" class="btn btn-sm btn-danger {{ $customer?->coBuyer?->first_name ? '' : 'd-none' }}" title="Delete Co-Buyer" onclick="deleteCoBuyer()">
            <i class="ti ti-trash"></i>
        </button>
    </div>

    <div id="editCoBuyerFields" class="{{ $customer?->coBuyer?->first_name ? '' : 'd-none' }}" data-main-address="{{ $customer->address ?? $customer->full_address ?? '' }}" data-main-city="{{ $customer->city ?? '' }}" data-main-state="{{ $customer->state ?? '' }}" data-main-zip="{{ $customer->zip_code ?? $customer->zip ?? '' }}">
        <div class="col-12">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
              ...
              <div class="col-md-6">
                  ...
                  <button class="btn btn-sm btn-dark" type="button" onclick="copyFromMainCustomer()">Copy From Main Customer</button>
    ...
    <input type="hidden" name="co_confirm" id="co_confirm" value="{{ $customer?->coBuyer?->confirmed ?? 0 }}">
...
<script>
    function openCustomerSocialModal(platform, customerId) {
        const id = customerId || window.AppState?.customerId || document.querySelector('[data-customer-id]')?.dataset.customerId;
        if (!id) {
            Swal.fire({ title: 'No customer selected', text: 'Please open a customer profile first.', icon: 'warning' });
            return;
        }
        ...
        function initActions(id, platform, modal) {
            ...
            if (saveBtn) saveBtn.onclick = async function () {
                ...
                if (url !== '' && !/^https?:\/\//i.test(url)) {
                    Swal.fire({ title: 'Invalid URL', text: 'Please enter a valid URL starting with http:// or https://', icon: 'warning' });
                    return;
                }
                ...
                try {
                    ...
                    console.log('Social save response status:', res.status, 'content-type:', res.headers.get('content-type'));
                    ...
                    if (res.ok) {
                        ...
                        if (ok) {
                            if (window.customerSocial) window.customerSocial[platform] = (data && data.url) ? data.url : url;
                            Swal.fire({ title: 'Saved', icon: 'success', timer: 1000, showConfirmButton: false });
                            try { modal.hide(); } catch (e) { const modalEl2 = document.getElementById('customerSocialModal'); const mi = modalEl2 ? bootstrap.Modal.getInstance(modalEl2) : null; if (mi) mi.hide(); }
                        } else {
                            const errMsg = (data && (data.message || JSON.stringify(data))) || text || 'Unknown server response';
                            console.error('Unexpected save response', res.status, errMsg);
                            Swal.fire({ title: 'Error', text: errMsg, icon: 'error' });
                        }
                    } else {
                        const errMsg = (data && (data.message || JSON.stringify(data))) || text || `Server returned ${res.status}`;
                        console.error('Save social error', res.status, errMsg);
                        Swal.fire({ title: 'Error', text: errMsg, icon: 'error' });
                    }
                } catch (e) {
                    console.error('Save social exception', e);
                    Swal.fire({ title: 'Error', text: 'Failed to save social link. See console for details.', icon: 'error' });
                } finally {
...
@include('customers.partials.general_customer_notes', ['customerId' => $customerId])
    ...
    {{-- Deals Section --}}
    @include('customers.partials.deals-section', ['customerId' => $customerId,'deals'=>$customer->deals ,'users' => $users ?? []])
...
<div class="offcanvas-footer bg-white p-3">
    <div class="d-flex gap-2">
        <button type="button" class="btn btn-light border border-1 flex-fill" data-bs-dismiss="offcanvas">Cancel</button>
...
<script>
    function toggleEditCoBuyer() {
        ...
        // Update toggle text
        if (toggleBtn) toggleBtn.textContent = visible ? 'Hide Co-Buyer' : (document.getElementsByName('co_first_name')[0]?.value ? 'Edit Co-Buyer' : '+ Co-Buyer');
    }
    ...
    async function confirmCoBuyer() {
        ...
        if (btn) { btn.disabled = true; btn.innerHTML = '<i class="ti ti-loader rotate"></i>'; }
    ...
    function deleteCoBuyer() {
        ...
        const doDelete = async () => {
            try {
                const res = await fetch(`/customers/${id}/cobuyer`, {
                    method: 'DELETE',
                    headers: { 'X-CSRF-TOKEN': window.AppState?.csrfToken || document.querySelector('meta[name="csrf-token"]')?.content },
                    credentials: 'same-origin'
                });
                const data = await res.json().catch(()=>null);
                if (res.ok && data && data.success) {
