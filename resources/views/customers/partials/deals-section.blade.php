                ...
                <div class="deal-box mb-3 p-2 border rounded bg-white shadow-sm" data-deal-id="{{ $deal->id }}"
                    ...
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <small class="text-muted small mb-0 d-flex align-items-center flex-wrap gap-1">
                                {{-- Created date --}}
                                @can('Edit Created Date')
                                    <span class="editable-date" style="text-decoration:underline;cursor:pointer;"
                                          onclick="startEditDealDate(this, {{ $deal->id }}, 'created_at', '{{ $deal->created_at->format('Y-m-d') }}')">Created: {{ $deal->created_at->format('F d, Y') }}</span>
                                @else
                                    <span>Created: {{ $deal->created_at->format('F d, Y') }}</span>
                                @endcan

                                {{-- Sold date --}}
                                @if(!empty($deal->sold_date) && (strtolower($deal->sales_status ?? '') === 'sold' || strtolower($deal->sales_status ?? '') === 'delivered'))
                                    @can('Edit Sold Date')
                                        <span class="ms-2 deal-sold-date editable-date" style="text-decoration:underline;cursor:pointer;"
                                              onclick="startEditDealDate(this, {{ $deal->id }}, 'sold_date', '{{ $deal->sold_date }}')">Sold: {{ \Carbon\Carbon::parse($deal->sold_date)->format('F d, Y') }}</span>
                                    @else
                                        <span class="ms-2 deal-sold-date">Sold: {{ \Carbon\Carbon::parse($deal->sold_date)->format('F d, Y') }}</span>
                                    @endcan
                                @endif

                                {{-- Delivered date --}}
                                @if(!empty($deal->delivery_date) && strtolower($deal->sales_status ?? '') === 'delivered')
                                    @can('Edit Delivery Date')
                                        <span class="ms-2 deal-delivery-date editable-date" style="text-decoration:underline;cursor:pointer;"
                                              onclick="startEditDealDate(this, {{ $deal->id }}, 'delivery_date', '{{ $deal->delivery_date }}')">Delivered: {{ \Carbon\Carbon::parse($deal->delivery_date)->format('F d, Y') }}</span>
                                    @else
                                        <span class="ms-2 deal-delivery-date">Delivered: {{ \Carbon\Carbon::parse($deal->delivery_date)->format('F d, Y') }}</span>
                                    @endcan
                                @endif
                            </small>
                        </div>
...
<!-- Sold Date Modal -->
<div class="modal fade" id="soldDateModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-sm modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Sold Date</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="mb-2">
                    <label class="form-label">Please select Sold Date.</label>
                    <input id="soldDateInput" type="date" class="form-control form-control-sm" />
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                <button id="soldDateSaveBtn" type="button" class="btn btn-primary">Save</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    async function updateDealField(dealId, field, value) {
        ...
        if (field === 'sales_status' && ['sold', 'delivered'].includes(String(value).toLowerCase())) {
            ...
            const modal = document.getElementById('soldDateModal');
            const input = document.getElementById('soldDateInput');
            const saveBtn = document.getElementById('soldDateSaveBtn');

            const isSold = String(value).toLowerCase() === 'sold';

            // set modal title/label dynamically
            const titleEl = modal.querySelector('.modal-title');
            const labelEl = modal.querySelector('.modal-body .form-label');
            titleEl.textContent = isSold ? 'Sold Date' : 'Delivery Date';
            labelEl.textContent = isSold ? 'Please select Sold Date.' : 'Please select Delivery Date.';
            ...
            input.value = max; // default to today
            ...
            const onSave = async function() {
                ...
                try {
                    const payloadKey = isSold ? 'sold_date' : 'delivery_date';
                    await api(`/api/deals/${dealId}/field`, 'PATCH', { field, value, [payloadKey]: selectedDate });
                    showToast('Updated');

                    // update UI: append date next to created date
                    const box = document.querySelector(`[data-deal-id="${dealId}"]`);
                    if (box) {
                        const createdEl = box.querySelector('.deal-vehicle-name')?.closest('.d-flex')?.querySelector('small.text-muted');
                        const createdSmall = createdEl || box.querySelector('small.text-muted');
                        if (createdSmall) {
                            let dateSpan;
                            if (isSold) {
                                dateSpan = box.querySelector('.deal-sold-date');
                                if (!dateSpan) {
                                    dateSpan = document.createElement('small');
                                    dateSpan.className = 'text-muted ms-2 deal-sold-date small';
                                    createdSmall.parentNode.appendChild(dateSpan);
                                }
                                const sd = new Date(selectedDate + 'T00:00:00');
                                const months = ['January','February','March','April','May','June','July','August','September','October','November','December'];
                                dateSpan.textContent = ` Sold: ${months[sd.getMonth()]} ${sd.getDate()}, ${sd.getFullYear()}`;
                            } else {
                                dateSpan = box.querySelector('.deal-delivery-date');
                                if (!dateSpan) {
                                    dateSpan = document.createElement('small');
                                    dateSpan.className = 'text-muted ms-2 deal-delivery-date small';
                                    createdSmall.parentNode.appendChild(dateSpan);
                                }
                                const sd = new Date(selectedDate + 'T00:00:00');
                                const months = ['January','February','March','April','May','June','July','August','September','October','November','December'];
                                dateSpan.textContent = ` Delivered: ${months[sd.getMonth()]} ${sd.getDate()}, ${sd.getFullYear()}`;
                            }
                        }
                    }
    ...
    function startEditDealDate(span, dealId, field, currentVal) {
        ...
        input.style.cssText = 'width:145px;';

        // Only restrict future for sold_date and created_at; delivery can be future
        if (field === 'sold_date' || field === 'created_at') {
        ...
        const finish = async (save) => {
            if (saving) return;
            if (save && input.value && input.value !== currentVal) {
                saving = true;
                try {
                    await api(`/api/deals/${dealId}/field`, 'PATCH', { field, value: input.value });
                    showToast('Date updated');
                    currentVal = input.value;
                    const d = new Date(input.value + 'T00:00:00');
                    const months = ['January','February','March','April','May','June','July','August','September','October','November','December'];
                    const formatted = `${months[d.getMonth()]} ${d.getDate()}, ${d.getFullYear()}`;
                    const label = field === 'created_at' ? 'Created' : (field === 'sold_date' ? 'Sold' : 'Delivered');
                    span.innerHTML = `${label}: ${formatted}`;
                } catch(e) {
                    showToast('Update failed', 'error');
        ...
        input.addEventListener('blur', () => setTimeout(() => { if (span.contains(input)) finish(false); }, 150));
