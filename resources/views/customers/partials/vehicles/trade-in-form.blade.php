...
<form id="tradeInForm" method="POST" action="{{ route('trade-ins.store') }}" enctype="multipart/form-data">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input type="number"
                   ...
                   id="odometerInput"
                   name="odometer"
                   placeholder="Enter Odometer in KM"
                   value="{{ $tradeIn->odometer ?? old('odometer') }}"
                   required>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="datetime-local"
                   class="form-control"
                   name="appraisal_datetime"
                   value="{{ $tradeIn->appraisal_datetime ?? old('appraisal_datetime') }}"
                   required>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="number"
                   ...
                   value="{{ $tradeIn->trade_allowance ?? old('trade_allowance') }}"
                   required>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="number"
                   ...
                   id="lienPayout"
                   name="lien_payout"
                   placeholder="Enter Lien Payout"
                   value="{{ $tradeIn->lien_payout ?? old('lien_payout') }}">
        </div>
        ...
        <div class="col-md-4">
            ...
            <input type="number"
                   ...
                   value="{{ $tradeIn->market_value ?? old('market_value') }}">
        </div>
        ...
        <div class="col-12 mt-4">
            <div class="d-flex gap-3 justify-content-between align-items-center flex-wrap">
                ...
                <button type="button"
                        ...
                        data-bs-target="#serviceAppointmentModal">
  ...
  <div class="modal fade" id="serviceAppointmentModal" tabindex="-1"
        aria-labelledby="serviceAppointmentModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="serviceAppointmentForm">
                        <div class="row g-3">
                            ...
                            <!-- Internal Department -->
                            <div class="col-md-6">
                                <label class="form-label">Internal Department <span
                ...
                @endif
                            ...
                            <div class="col-md-6">
                                <label class="form-label">Video Walkaround</label>
                                <input type="file" class="form-control" id="serviceVideoWalkaround"
                                    accept="video/*">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="file" class="form-control" id="servicePhotos" accept="image/*"
                                    multiple>
        ...
        document.getElementById('saveServiceAppointment')?.addEventListener('click', function() {
            ...
            if (!form.checkValidity()) {
                ...
                const totalVal = totalValEl && totalValEl.value ? parseFloat(totalValEl.value) || (laborVal + partsVal) : (laborVal + partsVal);
            ...
            const csrfToken = (window.AppState && window.AppState.csrfToken) || document.querySelector('meta[name="csrf-token"]')?.content || '';
            fetch('/api/service-appointment/create', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        } catch (e) { console.warn('Could not hide serviceAppointmentModal', e); }
                        form.reset();
                    } else {
...
document.getElementById('decodeVinBtn')?.addEventListener('click', async function() {
    ...
    try {
        const response = await api(`/api/vin/decode/${vin}`);
        if (response.success) {
            document.getElementById('vehicleYear').value = response.data.year || '';
            document.getElementById('vehicleMake').value = response.data.make || '';
            document.getElementById('vehicleModel').value = response.data.model || '';
            showToast('VIN decoded successfully');
        } else {
            showToast(response.message || 'Failed to decode VIN', 'error');
        }
    } catch (error) {
        showToast('Failed to decode VIN', 'error');
    } finally {
        this.disabled = false;
        this.innerHTML = '<i class="ti ti-search"></i> Decode';
    }
});
...
document.getElementById('tradeInForm')?.addEventListener('submit', async function(e) {
    ...
    const hasFiles = Array.from(formEl.querySelectorAll('input[type="file"]')).some(i => i.files && i.files.length > 0);
    ...
    try {
        ...
        if (contentType.includes('application/json')) {
            ...
            try { data = text ? JSON.parse(text) : null; } catch(e) { data = { success: false, text }; }
        }
