...
(function () {
	...
	// Datatable - validate table structure before initializing to avoid DT errors
	if ($('.datatable').length > 0) {
		$('.datatable').each(function () {
			var $tbl = $(this);

			// Skip manual-initialized tables
			if ($tbl.hasClass('no-dt-auto')) return;
			try {
				// Count only the first header row's TH elements (ignore filter/header helper rows)
				var thCount = $tbl.find('thead tr:first-child th').length;
				if (thCount === 0) {
					console.warn('Skipping DataTable init: no <th> found for table', $tbl);
					return;
				}

				var mismatch = false;
				$tbl.find('tbody tr').each(function (i) {
					// count visible cell elements (td or th)
					var tdCount = $(this).children('td,th').length;
					if (tdCount !== thCount) {
						mismatch = true;
						console.error('DataTable column count mismatch on table', $tbl, 'row index', i, 'expected', thCount, 'found', tdCount);
						return false; // break each
					}
				});

				if (mismatch) {
					// Leave the table as-is so the page still renders; log for debugging
					$tbl.addClass('datatable-skip-init');
					return;
				}

				// Safe to initialize this table
				$tbl.DataTable({
					"bFilter": true,
					"sDom": 'fBtlpi',
					"ordering": true,
					...
					initComplete: function (settings, json) {
						// Append filter inputs for legacy layout if those containers exist
						$('.dataTables_filter').appendTo('#tableSearch');
						$('.dataTables_filter').appendTo('.search-input');
					}
				});
			} catch (e) {
				console.error('Error initializing DataTable for', this, e);
			}
		});
	}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('table').forEach(table => {
    // Find the thead checkbox that is meant for "select all"
    const selectAll = table.querySelector('thead .form-check-input#select-all');

    if (!selectAll) return; // Skip tables without a select-all

    // Find only the checkboxes in tbody, ignore other checkboxes
    const rowCheckboxes = table.querySelectorAll('tbody .form-check-input');
    ...
    // Row checkbox change - update header checkbox status
    rowCheckboxes.forEach(cb => {
      cb.addEventListener('change', () => {
        ...
        selectAll.indeterminate = !allChecked && someChecked; // Shows "-" if some checked
      });
    });
  });
});
